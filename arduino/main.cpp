...
public:
  Status(byte size, State grip)
      : checkList(size), statusList(size + 1) {
    this->size = size;
    this->grip = grip;
  }
...
public:
  Parameter(Buffer& history, Status& st)
      : history(history), status(st) {}
  ...
  State checkPersistent(float newValue) {
    State currentStatus = this->status.checkValueStatus(newValue);

    if (currentStatus == this->status.getGrip()) {
      this->isAlertActive = false;
      return this->status.getGrip();
    }
...
const char* getStatusString(State status) {
  switch (status) {
    case State::GOOD:
      return "normal";
...
void loop() {
  ...
  float humid = humidity.getBuffer().getSmothedValue(
      dht11.readHumidity()
  );
  ...
  if (newTime - lastTime > 2000) {
    ...
    Serial.print(F(",\"temperature_status\":\""));
    Serial.print(getStatusString(statusTemp));
    Serial.print(F("\""));

    Serial.print(F(",\"humidity_status\":\""));
    Serial.print(getStatusString(statusHumid));
    Serial.print(F("\""));
