...
const controlServer = createServer(async (request, response) => {
    ...
    const url = new URL(request.url ?? "/", "http://simulator.local");
    if (request.method === "GET" &&
        url.pathname.startsWith("/sessions/")) {
        const deviceId = decodeURIComponent(url.pathname.slice("/sessions/".length));
    ...
    try {
        if (url.pathname === "/sessions/start") {
            ...
            const session = startSession(deviceId, requestedScenario, sendReading);
...
console.log(`[SIMULATOR] Control server listening on port ${controlPort}`);
